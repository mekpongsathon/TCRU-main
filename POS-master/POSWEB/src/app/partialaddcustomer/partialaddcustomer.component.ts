...
import { SelectItem, DialogService, DynamicDialogRef, DynamicDialogConfig } from 'primeng/api';
...
@Component({
  selector: 'app-partialaddcustomer',
  templateUrl: './partialaddcustomer.component.html',
  styleUrls: ['./partialaddcustomer.component.css']
})
export class PartialaddcustomerComponent implements OnInit {
  ...
  constructor(
    ...
    this._BusinessData = this._config.data;
  }
    ...
    if (this._BusinessData.id == 0) {
      this._businessservice.Insert(this._BusinessData).subscribe((res: ApiReposeModel) => {
        if (res.Type == "S") {
          ...
          this._ref.close({"Status" : true, "BusinessId" : res.Id});
        }
        else {
